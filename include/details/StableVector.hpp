...
template <typename, typename> class PepperedVector;

template <typename, typename> class CompactMap;

template <typename T, typename RefTag = DefRefTag> struct Ref {
private:
  std::size_t id = 0;
...
template <typename T, typename RefTag = DefRefTag> class PepperedVector {
private:
...
public:
  ...
  template <typename E> Ref insert(E &&elem) { // forwarded
    Ref ind;
  ...
  [[nodiscard]] const T &operator[](const Ref &ind) const {
...
template <typename T, typename RefTag = DefRefTag> class CompactMap {
private:
  using Ref = Ref<T, RefTag>;
  struct Elem {
    Ref ind;
...
template <typename T, typename RefTag> struct std::hash<Ref<T, RefTag>> {
  std::size_t operator()(const Ref<T, RefTag> &ref) const {
