...
class BlockRenderer {
  ...
  void setViewDefault() {
    vsScale =
        static_cast<float>(std::min(window.getSize().x, window.getSize().y)) /
        defaultViewSize;
...
public:
  BlockRenderer(Block &blck_, sf::RenderWindow &window_, const sf::Font &font_)
    ...
    for (std::size_t x = 0; x < block.size; ++x) {
      for (std::size_t y = 0; y < block.size; ++y) {
        ...
        gridVertecies[index + 1] = {pos - sf::Vector2f{crossSize, 0.0f},
                                    crossCol};
        gridVertecies[index + 2] = {pos - sf::Vector2f{0.0f, -crossSize},
                                    crossCol};
        gridVertecies[index + 3] = {pos - sf::Vector2f{0.0f, crossSize},
                                    crossCol};
      }
    }
    ...
    sf::Vertex tr = {
        {static_cast<float>(block.size), static_cast<float>(block.size)}};
  ...
  void event(const sf::Event &event, const sf::Vector2f &mousePos) {
    if (event.type == sf::Event::MouseWheelMoved &&
      ...
      float zoom =
          static_cast<float>(std::pow(zoomFact, -event.mouseWheel.delta));
...
private:
  void draw(const sf::Vector2f &mousePos) {
    ...
    window.draw(gridVertecies.data(), gridVertecies.size(),
